...
describe('crypto op codes', async () => {
  ...
  describe('ed25519verify', async () => {
    ...
    test('should throw error when no active txn group', async () => {
      expect(() => op.ed25519verify(Bytes(''), Bytes<64>(), Bytes<32>())).toThrow('no active txn group')
    })
  })
  ...
  describe('ecdsaVerify', async () => {
    test('should be able to verify k1 signature', async ({ appClientCryptoOpsContract: appClient }) => {
      ...
      const pubkeyX = Bytes.fromHex<32>('a710244d62747aa8db022ddd70617240adaf881b439e5f69993800e614214076')
    ...
    test('should be able to verify r1 signature', async ({ appClientCryptoOpsContract: appClient }) => {
      const messageHash = Bytes.fromHex<32>('f809fd0aa0bb0f20b354c6b2f86ea751957a4e262a546bd716f34f69b9516ae1')
      const sigR = Bytes.fromHex<32>('18d96c7cda4bc14d06277534681ded8a94828eb731d8b842e0da8105408c83cf')
      const sigS = Bytes.fromHex<32>('7d33c61acf39cbb7a1d51c7126f1718116179adebd31618c4604a1f03b5c274a')
      const pubkeyX = Bytes.fromHex<32>('f8140e3b2b92f7cbdc8196bc6baa9ce86cf15c18e8ad0145d50824e6fa890264')
      const pubkeyY = Bytes.fromHex<32>('bd437b75d6f1db67155a95a0da4b41f2b6b3dc5d42f7db56238449e404a6c0a3')
  ...
  describe('ecdsaPkDecompress', async () => {
    test('should be able to decompress k1 public key', async ({ appClientCryptoOpsContract: appClient }) => {
      ...
      const keyPair = ecdsa.keyFromPublic(asUint8Array(testData.pubkeyX.concat(testData.pubkeyY)))
  ...
  describe('vrfVerify', async () => {
    ...
    test('should return mocked result', async ({ appClientCryptoOpsContract: appClient }) => {
      ...
      const mockedVrfVerify = op.vrfVerify as Mock<typeof op.vrfVerify>
      mockedVrfVerify.mockReturnValue([Bytes<64>(new Uint8Array(avmResult[0])), avmResult[1]])
...
const generateEcdsaTestData = (v: Ecdsa) => {
  ...
  return {
    data: Bytes<32>(new Uint8Array(messageHash)),
    r: Bytes<32>(new Uint8Array(signature.r.toArray('be', 32))),
    s: Bytes<32>(new Uint8Array(signature.s.toArray('be', 32))),
