...
export class GlobalMapCls<TKey, TValue> {
  ...
  call(proxy: GlobalMapCls<TKey, TValue>, key: TKey): GlobalStateType<TValue> {
...
export function GlobalMap<TKey, TValue>(options?: { keyPrefix?: bytes | string }): GlobalMapType<TKey, TValue> {
  ...
  const func = (key: TKey): GlobalStateType<TValue> => globalMap.call(proxy, key)
...
export class LocalStateCls<ValueType> {
  ...
  call(proxy: LocalStateCls<ValueType>, account: Account): LocalStateForAccount<ValueType> {
    return this.map.getValue(proxy.key, account)
  }
}
...
export function LocalState<ValueType>(options?: { key?: bytes | string }): LocalStateType<ValueType> {
  ...
  const func = (account: Account): LocalStateForAccount<ValueType> => localState.call(proxy, account)
...
export class LocalMapCls<TKey, TValue> {
  ...
  call(proxy: LocalMapCls<TKey, TValue>, key: TKey, account?: Account): LocalStateForAccount<TValue> | LocalStateType<TValue> {
    const fullKey = proxy.keyPrefix.concat(toBytes(key))
    return account
...
export function LocalMap<TKey, TValue>(options?: { keyPrefix?: bytes | string }): LocalMapType<TKey, TValue> {
  ...
  const func = (key: TKey, account?: Account): LocalStateForAccount<TValue> | LocalStateType<TValue> => localMap.call(proxy, key, account)
